...
export const DEFAULT_VALIDATION_SYSTEM_PROMPT = [
  ...
  '6. You must preserve `tokenCompliance`, `hallucinationsCaught`, and `stateCoverage` exactly as implied by `deterministicSummary`.',
  '7. Use the generated code, component interfaces, unit tests, e2e tests, parsing result, gap analysis, and resolving-gaps decisions to assess accessibility basics and contract compatibility.',
  '8. `accessibilityScore` should be a short human-readable score or rating such as `basic_pass`, `needs_attention`, or `partial_pass`.',
  '9. `contractCompatibilityIssues` should list concrete integration or interface mismatches only when they are justified by the provided artifacts.',
  '10. `affectedComponentCodes`, `isRegenerationRequired`, and `regenerationReasons` must be consistent with the validation findings that justify selective regeneration.',
  '11. `issuesFound` must include important validation problems. If deterministicSummary already indicates missing states or hallucinated tokens, include those problems in `issuesFound`.',
  '12. Do not invent missing files, states, tokens, or component contracts.',
  '13. Keep the output concise and implementation-oriented.',
  '14. Do not mention JSON, schema, prompts, or internal process language in the output.',
  '',
...
export const DEFAULT_VALIDATION_USER_TEMPLATE = [
  ...
  'Use deterministicSummary as the source of truth for hallucinated tokens, token compliance, and state coverage. Use the rest of the provided artifacts to evaluate accessibility basics and contract compatibility.',
  'Before answering, verify that all required fields are present, no extra properties exist, and the deterministic validation fields match the provided deterministicSummary.',
  '',
  'Input JSON:',
  '{{input}}',
].join('\n');
